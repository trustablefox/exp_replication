    ...
    def extract_mus(self, start_from=None):
        ...
        for i, p in enumerate(self.assums):
            ...
            if sat:
                ...
                #    self.enc.printLits(to_test)
                #    print("UNSAT")
        expl_ = [i for i in self.assums if i > 0]
        self.assums = [abs(i) for i in self.assums]
        '''

        '''
        if self.verbose:
            print('')
            print('#SAT: {0} | #UNSAT: {1}'.format(len(self.stimes), len(self.utimes)))
            if(self.nsat):
                print('SAT: tot: {0:.2f} | m: {1:.2f} | M: {2:.2f} | avg: {3:.2f}'.format(
                    sum(self.stimes), min(self.stimes), max(self.stimes), sum(self.stimes) / len(self.stimes)))
            if(self.nunsat):
                print('UNSAT: tot: {0:.2f} | m: {1:.2f} | M: {2:.2f} | avg: {3:.2f}'.format(
                    sum(self.utimes), min(self.utimes), max(self.utimes), sum(self.utimes) / len(self.utimes)))
            print('')
        '''
